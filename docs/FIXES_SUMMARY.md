...
本次修复解决了代码审查中发现的所有 P0、P1 和 P2 级别问题，以及补充与第三轮加固（配置化与端点收敛）内容。
...
6. **可配置与部署易用**: 新增图像/下载安全配置项；Compose/Helm/README/.env 示例已对齐；`v0.2.0` 版本发布可固定部署。
...
4. **图像限幅**: 默认 8192x8192，建议按业务数据适度下调（如 4096），平衡质量与内存占用。
...
4. 对大 PDF 的自动分批与资源使用进行压测，并扩展测试覆盖。
...
- Tag: `v0.2.0`
- 主要变更：安全加固（路径/扩展名/长度/越界写入防护）、图像尺寸限幅可配置、测试与文档/部署配置更新。
